...
ir::Value ir::Executor::find_src_operand(Operand op) {
    ...
    retval = iter->second;
    if(op.type==Type::Int){
         assert(retval.t == op.type && "type not match op.type==int");
    }else  if(op.type==Type::IntLiteral){
         assert(retval.t == op.type && "type not match op.type==IntLiteral");
    }else  if(op.type==Type::IntPtr){
         assert(retval.t == op.type && "type not match op.type==IntPtr");
    }else  if(op.type==Type::Float){
        if(retval.t==Type::Int) assert(retval.t == op.type && "type not matchop.type== Float,retval.t==Type::Int");
        if(retval.t==Type::IntLiteral) assert(retval.t == op.type && "type not match op.type==Float,retval.t==Type::IntLiteral");
        if(retval.t==Type::IntPtr) assert(retval.t == op.type && "type not match op.type==Float,retval.t==Type::IntPtr");
        if(retval.t==Type::Float) assert(retval.t == op.type && "type not match op.type==Float,retval.t==Type::Float");
        if(retval.t==Type::FloatPtr) assert(retval.t == op.type && "type not match op.type==Float,retval.t==Type::FloatPtr");
        if(retval.t==Type::FloatLiteral) assert(retval.t == op.type && "type not match op.type==Float,retval.t==Type::FloatLiteral");
        //  assert(retval.t == op.type && "type not match Float");
    }else  if(op.type==Type::FloatPtr){
         assert(retval.t == op.type && "type not match op.type==FloatPtr");
    }else  if(op.type==Type::FloatLiteral){
         assert(retval.t == op.type && "type not match op.type==FloatLiteral");
    }else{
        assert(retval.t == op.type && "type not match op.type==null");
    }
...
bool ir::Executor::exec_ir(size_t n) {
    ...
    std::cout << "\tin goto: pc = " << cur_ctx->pc << std::endl;
            ...
            case Operator::call: {
                ...
                if (cxt) {
                    ...
                    for (size_t i = 0; i < cxt->pfunc->ParameterList.size(); i++) {
                        ...
                        if((para).type==Type::Int) assert(para.type == Type::Float&& "para:Int");
                        if((para).type==Type::IntLiteral) assert(para.type == Type::Float&& "para:IntLiteral");
                        if((para).type==Type::Float) assert(para.type == Type::Float && "para:Float");

                        if((para).type==Type::FloatLiteral) assert(para.type == Type::Float && "para:FloatLiteral");
            ...
            case Operator::store: {
                ...
                if (IS_INT_OPERAND(inst->des) && inst->op1.type == Type::IntPtr) {
                    ...
                    // if(!IS_FLOAT_OPERAND(inst->des) ，inst->op1.type = Type::IntPtr)assert(0 && "!IS_FLOAT_OPERAND(inst->des)");
                     if(inst->des.type==Type::FloatLiteral&&inst->op1.type != Type::FloatPtr)assert(0 && "inst->op1.type != Type::FloatPtr");
                ...
                if(!IS_INT_OPERAND(inst->des)&&inst->op1.type != Type::IntPtr)assert(0 && "!IS_INT_OPERAND(inst->des)");
            ...
            case Operator::mov: 
            case Operator::def: {
                // assert(IS_INT_OPERAND(inst->des)&& "def相关操作出问题");
                if((inst->des).type == Type::Float)assert(((inst->des).type == Type::Int || (inst->des).type == Type::IntLiteral)&& "def-wrongFloat!");
                else if((inst->des).type == Type::FloatLiteral)assert(((inst->des).type == Type::Int || (inst->des).type == Type::IntLiteral)&& "def-wrongFloatLiteral!");
                else assert(((inst->des).type == Type::Int || (inst->des).type == Type::IntLiteral)&& "def-wrong!");
            ...
            case Operator::_or: 
            {
                switch (inst->op) {  // 假设 inst->op 是包含操作符类型的变量
                    case Operator::add:
                        std::cout << "Operator::add";
                        assert(inst->des.type == Type::Int && "add");
                    case Operator::sub:
                        std::cout << "Operator::sub";
                        assert(inst->des.type == Type::Int && "sub");
                    case Operator::mul:
                        std::cout << "Operator::mul";
                        assert(inst->des.type == Type::Int && "mul");
                    case Operator::div:
                        std::cout << "Operator::div";
                        assert(inst->des.type == Type::Int && "div");
                    case Operator::mod:
                        std::cout << "Operator::mod";
                        assert(inst->des.type == Type::Int && "mod");
                    case Operator::lss:
                        std::cout << "Operator::lss";
                        assert(inst->des.type == Type::Int && "lss");
                    case Operator::leq:
                        std::cout << "Operator::leq";
                        assert(inst->des.type == Type::Int && "leq");
                    case Operator::gtr:
                        std::cout << "Operator::gtr";
                        assert(inst->des.type == Type::Int && "gtr");
                    case Operator::geq:
                        std::cout << "Operator::geq";
                        assert(inst->des.type == Type::Int && "geq");
                    case Operator::eq:
                        std::cout << "Operator::eq";
                        assert(inst->des.type == Type::Int && "eq");
                    case Operator::neq:
                        std::cout << "Operator::neq";
                        assert(inst->des.type == Type::Int && "neq");
                    case Operator::_and:
                        std::cout << "Operator::_and";
                        assert(inst->des.type == Type::Int && "and");
                    case Operator::_or:
                        std::cout << "Operator::_or";
                        assert(inst->des.type == Type::Int && "or");
                    default:
                        std::cout << "Unknown operator";
                        assert(inst->des.type == Type::Int && "Operand type must be Type::Unknown");
                }
                // assert(inst->des.type == Type::Int);
                // op1
                int v1;
                if (IS_INT_OPERAND(inst->op1)) {
                    ...
                    // assert(0 && "type of op1 is not right type");
                    switch (inst->op) {  // 假设 inst->op 是包含操作符类型的变量
                    case Operator::add:
                        std::cout << "Operator::add";
                        assert(0 && "type of add is not right type");
                    case Operator::sub:
                        std::cout << "Operator::sub";
                    ...
                    case Operator::mod:
                        std::cout << "Operator::mod";
                        assert(0 && "type of mod is not right type");
                    case Operator::lss:
                        std::cout << "Operator::lss";
                        assert(0 && "type of lss is not right type");
                    case Operator::leq:
                        std::cout << "Operator::leq";
                        assert(0 && "type of leq is not right type");
                    case Operator::gtr:
                        std::cout << "Operator::gtr";
                        assert(0 && "type of gtr is not right type");
                    case Operator::geq:
                        std::cout << "Operator::geq";
                        assert(0 && "type of geq is not right type");
                    case Operator::eq:
                        std::cout << "Operator::eq";
                        assert(0 && "type of eq is not right type");
                    case Operator::neq:
                        std::cout << "Operator::neq";
                        assert(0 && "type of neq is not right type");
                    case Operator::_and:
                        std::cout << "Operator::_and";
                        assert(0 && "type of _and is not right type");
                    case Operator::_or:
                        std::cout << "Operator::_or";
                        assert(0 && "type of op1 is not right type");
                    default:
                ...
                if (IS_INT_OPERAND(inst->op2)) {
                   ...
                   // assert(0 && "type of op2 is not right type");
                     switch (inst->op) {  // 假设 inst->op 是包含操作符类型的变量
                    case Operator::geq:
                        std::cout << "Operator::geq";
                        assert(0 && "op2-type of geq is not right type");
                    case Operator::eq:
                        std::cout << "Operator::eq";
                        assert(0 && "op2-type of eq is not right type");
                    case Operator::neq:
                        std::cout << "Operator::neq";
                        assert(0 && "op2-type of neq is not right type");
                    case Operator::_and:
                        std::cout << "Operator::_and";
                        assert(0 && "op2-type of _and is not right type");
                    case Operator::_or:
                        std::cout << "Operator::_or";
                        assert(0 && "op2-type of _or is not right type");
                    default:
                        std::cout << "Unknown operator";
                        assert(inst->des.type == Type::Int && "Operand type must be Type::Unknown");
                        }
            ...
            case Operator::fneq:
            {
                ...
                if (IS_FLOAT_OPERAND(inst->op2)) {
                    ...
                    // assert(0 && "type of op2 is not Type::FLoat or Type::FloatLiteral");

                    {
                    // assert(0 && "type of op1 is not right type");
                    switch (inst->op) {  // 假设 inst->op 是包含操作符类型的变量
                    case Operator::fadd:
                        std::cout << "Operator::fadd";
                        assert(0 && "type of fadd is not right type");
                    case Operator::fsub:
                        std::cout << "Operator::fsub";
                        assert(0 && "type of fsub is not right type");
                    case Operator::fmul:
                        std::cout << "Operator::fmul";
                        assert(0 && "type of fmul is not right type");
                    case Operator::fdiv:
                        std::cout << "Operator::fdiv";
                        assert(0 && "type of fdiv is not right type");

                    case Operator::flss:
                        std::cout << "Operator::flss";
                        if((inst->op2).type==Type::Int) assert(0 && "op2type:Int of flss is Type::FLoat or Type::FloatLiteral");
                        if((inst->op2).type==Type::IntLiteral) assert(0 && "op2type:IntLiteral of flss is Type::FLoat or Type::FloatLiteral");
                        if((inst->op2).type==Type::Float) assert(0 && "op2type:Float of flss is Type::FLoat or Type::FloatLiteral");

                        if((inst->op2).type==Type::FloatLiteral) assert(0 && "op2type:FloatLiteral of flss is Type::FLoat or Type::FloatLiteral");

                                                
                    case Operator::fleq:
                        std::cout << "Operator::fleq";
                        assert(0 && "type of fleq is not right type");
                    case Operator::fgtr:
                        std::cout << "Operator::fgtr";
                        assert(0 && "type of fgtr is not right type");
                    case Operator::fgeq:
                        std::cout << "Operator::fgeq";
                        assert(0 && "type of fgeq is not right type");
                    case Operator::feq:
                        std::cout << "Operator::feq";
                        assert(0 && "type of feq is not right type");
                    case Operator::fneq:
                        std::cout << "Operator::fneq";
                        assert(0 && "type of fneq is not right type");
                    case Operator::_and:
                        std::cout << "Operator::_and";
                        assert(0 && "type of _and is not right type");
                    case Operator::_or:
                    ...
                    default:
                        std::cout << "Unknown operator";
                        assert(inst->des.type == Type::Float && "Operand type must be Type::Unknown");}
                }
                }
                auto pvalue = get_des_operand(inst->des);
                switch (inst->op) {
