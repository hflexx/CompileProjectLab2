        ...
        for i in ["basic", "function"]:
            ...
            if os.path.exists(output_dir):
                ...
                for file in files:
                    ...
                    if cp.returncode != 0:
                        record[file] = {"retval1": cp.returncode, "err_detail": "diff test failed"}
                    else:
                        score += 1
                        record[file] = {"retval1": 0}
        ...
        for i in ["basic", "function"]:
            ...
            if os.path.exists(output_dir):
                ...
                for file in files:
                    ...
                    cp = subprocess.run(cmd, shell=True, stderr=subprocess.DEVNULL, stdout=subprocess.PIPE)
                    if cp.returncode != 0:
                        record[file] = {"retval2": cp.returncode, "err_detail": cp.stdout}
                    else:
                        score += 1
                        record[file] = {"retval2": 0}
                    print(file, record[file])
        print("score:",score,"/",total)
    elif step == "-s2":
        for i in ["basic", "function"]:
