...
def run_compiler(arg1):
    ...
    i = "function"  # 只处理 "function" 文件夹中的文件
    testcase_dir = testcase_base + i + '/'
    output_dir = output_base + i + '/'
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    ...
    file_to_run = "95_float.sy"
    fname, ftype = file_to_run.split('.')
    cmd = ' '.join([compiler_path, testcase_dir + file_to_run, step, "-o", output_dir + fname + "." + oftype])
    if is_windows:
        cmd = cmd.replace('/', '\\')
    cp = subprocess.run(cmd, shell=True, stderr=subprocess.PIPE, stdout=subprocess.DEVNULL)
    if cp.returncode != 0:
        record[file_to_run] = {"retval": cp.returncode, "err_detail": cp.stderr}
    else:
        record[file_to_run] = {"retval": 0}
    print(file_to_run, record[file_to_run])
